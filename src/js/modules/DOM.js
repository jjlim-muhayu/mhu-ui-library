class realDom {
    ...
    addClass(classList) {
        ...
        if (/\s/.test(classList)) {
            classList
                .split(' ')
                .forEach(classname => this.selector.classList.add(classname));
        } else {
    ...
    append(source, target) {
        ...
        if (isTagSource) {
            ...
            sourceTag = domParser.parseFromString(source, 'text/xml')
                .children[0];
        } else {
            ...
            tagArr.reduce((pre, acc, index) => {
                ...
                //if(/(\.)|(#)/.test(acc)){
                let str =
                    'div#firstLevelId.firstClass0.firstClass1>div.myClass.secondClass#idName>p#myId>h1.hClass>h2.sub.second > 얼렁뚱땅 넣기';
                ...
                function sliceClassId(str) {
                    ...
                    if (/#/.test(str)) {
                        if (str.match(/#/).index === 0) {
                            temp.id = str.slice(
                                1,
                                /\./.test(str) ? str.indexOf('.') : str.length,
                            );
                            temp.class = /\./.test(str)
                                ? str
                                      .slice(str.indexOf('.'))
                ...
                tempClass = {
                    ...
                    class: isIdClass
                        ? sliceClassId(acc.slice(strMatchIndex))
                        : '',
                };
                ...
                if (typeof tempClass.class === 'object') {
                    ...
                    if (tempClass.class.id)
                        createEl.setAttribute('id', tempClass.class.id);
                    if (tempClass.class.class.length)
                        createEl.setAttribute('class', tempClass.class.class);
                }
